		...
		<ThemedView style={styles.container}>
			...
			<ScrollView style={{ flex: 1 }} contentContainerStyle={styles.content}>
				{LANGS.map((x) => {
					...
					return (
						<Pressable
							...
							style={[styles.langOption, { backgroundColor: surface, borderColor: border }, selected ? styles.langSelected : null]}
						>
							<View style={styles.langRow}>
								<ThemedText style={styles.flag}>{x.flag}</ThemedText>
								<ThemedText style={styles.langName}>{x.name}</ThemedText>
							</View>
				...
				<View style={styles.noteBox}>
					<ThemedText style={styles.noteText}>{t(lang, 'language.applied')}</ThemedText>
				</View>
			</ScrollView>
		</ThemedView>
	);
...
const styles = StyleSheet.create({
	...
	langSelected: { borderColor: '#2C2D5F' },
	langRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
	flag: { fontSize: 28, lineHeight: 34 },
	langName: { fontSize: 16, lineHeight: 22, fontWeight: '700', color: '#2C2D5F' },
	check: { color: '#2C2D5F', fontSize: 18, fontWeight: '700' },

	noteBox: { marginTop: 12, borderRadius: 14, backgroundColor: '#EAF2FF', padding: 12 },
	noteText: { color: '#2C2D5F' },
});
